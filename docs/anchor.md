...
— specifically the `basics/program-structure` and `basics/idl` pages. Read
this before touching the on-chain program — it's the minimum mental model
needed to avoid fighting the framework.

## The four macros

Anchor's behavior is driven entirely by four macros. Everything else
(account validation, IDL generation, client codegen) falls out of them.
...
## File layout is *not* part of Anchor
...
code. The Anchor CLI scaffold ships an `instructions/initialize.rs` only
because the default scaffold instruction is `initialize`; renaming the
instruction frees us to rename the file.
...
- **Account types** (`Signer<'info>`, `Program<'info, T>`, `Account<'info, T>`,
  ...
  all (no owner, discriminator, or shape checks), requires explicit
  constraints plus a `/// CHECK:` comment, and should be avoided unless
  genuinely necessary.
- **`#[account(...)]` constraints** — extra invariants Anchor enforces before
  ...
  | `seeds = [...], bump` | declares this account as a PDA derived from the listed seeds. Anchor computes the canonical bump and stores it on `ctx.bumps`. |
  ...
  **Constraint order matters.** Anchor parses an `#[account(...)]` list in
  ...
  before `close`. Out-of-order constraints are a build error (e.g. `init must
  be provided before payer`). So write `init` / `init_if_needed` first, then
  its `payer` / `space` / `seeds = [...], bump`, then any `mut` / `has_one` /
  `constraint` checks.
...
- **Discriminator and `space` for instructions**: instruction discriminators
  use `sha256("global:<instruction_name>")`. Anchor injects them transparently;
  you do not need to do anything.

## IDL is generated, not written

`anchor build` writes `target/idl/<program>.json` and a TypeScript types file
at `target/types/<program>.ts`. The IDL is derived from the Rust source:
...
- `lib.rs` only contains `declare_id!` and the `#[program] mod fund { … }`
  block whose functions delegate to `<name>::handler`. Keep the bodies a
  one-liner.
- Account constraints on PDAs always store and reuse the bump (`bump = …`
  after creation; `bump` alone at `init` time). The stored bump goes on the
  parent account so descendants can re-derive without searching.
